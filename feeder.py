...
class VarFeeder:
    '''
    Replace this with TFRecords, it builds temporary TF graph, injects variables into, and saves variables to TF checkpoint.
    In a train time, variables can be built by build_vars() and content restored by FeederVars.restore()
    :param path: str - directory to store the data
    :param tensor_vars: Dict[str, Union[pd.DataFrame, pd.Series, np.ndarray]] - variables to save as tensors
    :param plain_vars: Dict[str, Any] - variables to save as Python objects
    '''
    def __init__(self, path, tensor_vars, plain_vars):
        ...
        with open(_meta_file(path), mode='wb') as f:
            pickle.dump(self, f)
        
        with tf.Graph().as_default():
            tensor_vars = self._build_vars()
            placeholders = [tf.placeholder(tf.as_dtype(dtype), shape=shape) for dtype, shape in zip(self.dtypes, self.shapes)]
            assigners = [tensor_var.assign(placeholder) for tensor_var, placeholder in zip(tensor_vars, placeholders)]
        ...
        def _build_vars(self):
            def make_tensor(shape, dtype, name):
                ...
                return tf.get_local_variable(name=name, initalizer=init, dtype=tf_type)
            
            with tf.device("/cpu:0"):
                with tf.name_scope('feeder_vars'):
                    return [make_tensor(shape, dtype, name) for shape, dtype, name in zip(self.shapes, self.dtypes, self.names)]
        ...
        @staticmethod
        def read_vars(path):
            with open(_meta_file(path), mode='rb') as f:
                feeder = pickle.load(f)
            assert feeder.path = path
            assert feeder.create_vars()


class FeederVars(UserDict):
    def __init__(self, tensors, plain_vars, path):
    ...
    def restore(self, session):
        '''
        Restore the content of the variables for this session
