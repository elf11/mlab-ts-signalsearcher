...
class ModelTrainerV2:
    ...
    def process_metrics(self, key, run_results, epoch, step):
        ...
        for result, metric in zip(run_results, metrics):
            ...
            tag_str = key.name + "/" + metric.name + "_0"
            summaries.append(tf.Summary.Value(tag=tag_str, simple_value=result))
        return summaries
...
class ModelTrainer:
    ...
    def process_eval_results(self, run_results, offset, global_step, epoch):
        ...
        mae, smape = results
        tag_mae = "test/MAE_" + self.model_no
        tag_smape = "test/SMAPE_" + self.model_no
        if self.summary_writer and global_step > 200:
            summary = tf.Summary(value=[
                tf.Summary.Value(tag=tag_mae, simple_value=mae),
                tf.Summary.Value(tag=tag_smape, simple_value=smape),
            ])
...
def train(name, hparams, multi_gpu=False, n_models=1, train_completeness_threshold=0.01,
    ...
    def create_model(scope, index, prefix, seed):
        ...
        if write_summaries:
            path_str = logdir + "/" + name + "_" + str(index)
            summ_path = path_str
    ...
    if n_models == 1:
        ...
        for i in range(n_models):
            gpu_name = "/gpu:" + str(gpu)
            gpu_i = "gpu:" + str(i)
            device = gpu_i if multi_gpu else gpu_name
            ...
            for _ in tqr:
                ...
                if step % eval_every_step == 0:
                    ...
                    if save_best_model and epoch > 0 and eval_smape.last < best_smape:
                        ...
                        s = "data/cpt/" + name + "/cpt"
                        saver.save(sess, s, global_step=step)
...
if __name__ == '__main__':
    ...
    result = train("definc_attn", hparams, n_models=1, train_sampling=1.0, eval_sampling=1.0, patience=5, multi_gpu=False,
                    save_best_model=False, gpu=0, eval_memsize=15, seed=5, verbose=True, forward_split=False,
                    write_summaries=True, side_split=True, do_eval=True, predict_window=63, asgd_decay=None, max_steps=11500,
