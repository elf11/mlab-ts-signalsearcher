...
class InputPipe:
    ...
    def cut_train(self, hits, *args):
        ...
        if self.verbose:
            lower_train_start = datetime.strptime(self.inp.data_start, '%Y-%m-%d') + timedelta(days=self.start_offset)
            lower_test_end = lower_train_start + timedelta(days=n_days)
            lower_test_start = lower_test_end - timedelta(days=self.predict_window)
            upper_train_start = datetime.strptime(self.inp.data_start, '%Y-%m-%d') + timedelta(days=free_space - 1)
            upper_test_end = upper_train_start + timedelta(days=n_days)
            upper_test_start = upper_test_end - timedelta(days=self.predict_window)
            ...
            print("Free space for training: " + str(free_space) + " days.")
            print(" Lower train " + str(lower_train_start) + ", prediction " + str(lower_test_start) + ".." + str(lower_test_end))
            print(" Upper train "+ str(upper_train_start) +", prediction" + str(upper_test_start) + ".." +str(upper_test_end))
    ...
    def make_features(self, x_hits, y_hits, dow, lagged_hits, page_ix,
        ...
        flat_page_features = tf.concat([stacked_features], axis=0)
    ...
    def __init__(self, inp: VarFeeder, features: Iterable[tf.Tensor], n_pages: int, mode: ModelMode, n_epoch=None,
        ...
        if mode == ModelMode.TRAIN:
            ...
            if verbose:
                train_start = datetime.strptime(inp.data_start, '%Y-%m-%d') + timedelta(days=self.start_offset)
                eval_start = train_start + timedelta(days=train_window)
                end = eval_start + timedelta(days=predict_window - 1)
...
def page_features(inp: VarFeeder):
    return (inp.hits, inp.page_ix, inp.page_popularity, inp.year_autocorr, inp.quarter_autocorr)
