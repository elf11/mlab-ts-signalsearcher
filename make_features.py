...
def find_start_end(data: np.ndarray):
    ...
    n_days = data.shape[1]
    start_idx = np.full(n_astimeseries, -1, dtype=np.int32)
    end_idx = np.full(n_astimeseries, -1, dtype=np.int32)
    for astimeseries in range(n_astimeseries):
        # scan from start to the end
        for day in range(n_days):
            if not np.isnan(data[astimeseries, day]) and data[astimeseries, day] > 0:
                start_idx[astimeseries] = day
                break
        # reverse scan, from end to start
        for day in range(n_days - 1, -1, -1):
            if not np.isnan(data[astimeseries, day]) and data[astimeseries, day] > 0:
                end_idx[astimeseries] = day
                break
    return start_idx, end_idx


def prepare_data(attr, start, end, valid_threshold) -> Tuple[pd.DataFrame, pd.DataFrame, np.ndarray, np.ndarray]:
    """
...
def run():
    ...
    # Get the data
    df, nans, starts, ends = prepare_data(args.attr, args.start, args.end, args.valid_threshold)
